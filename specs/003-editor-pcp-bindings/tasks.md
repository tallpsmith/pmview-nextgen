...
- [x] T018 [P] [US2] Create `PcpBindingInspectorPlugin` in `godot-project/addons/pmview-bridge/PcpBindingInspectorPlugin.cs` — `[Tool]` class extending `EditorInspectorPlugin`. `_CanHandle()` returns true for nodes that have a child `PcpBindable` or that themselves are `PcpBindable`. `_ParseEnd()` adds a "Browse Metrics" `Button` that opens `MetricBrowserDialog`. Store a single `MetricBrowserDialog` instance, lazily created and added to `EditorInterface.Singleton.GetBaseControl()`. **NOTE**: `_ParseProperty()` dynamic enum enhancement for `@export` vars deferred — the static enum hint on PcpBindingResource covers built-in vocabulary; custom properties pass through as typed strings. Dynamic enhancement can be added as a follow-up.
