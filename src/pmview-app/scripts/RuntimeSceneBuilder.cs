...
public static class RuntimeSceneBuilder
{
    ...
    private static void AddMetricPoller(Node3D root, string endpoint, string hostname)
    {
        ...
        poller.SetScript(GD.Load<Script>(MetricPollerScriptPath));
    ...
    private static void AddSceneBinder(Node3D root)
    {
        ...
        binder.SetScript(GD.Load<Script>(SceneBinderScriptPath));
    ...
    {
        var bindingScript = GD.Load<Script>(BindingResScriptPath);
        var res = new Resource();
        res.SetScript(bindingScript);
    ...
    {
        var bindableScript = GD.Load<Script>(BindableScriptPath);
        var bindingResScript = GD.Load<Script>(BindingResScriptPath);

        var bindable = new Node { Name = "PcpBindable" };
        bindable.SetScript(bindableScript);
